<?php 
/**
 * Members Locator "horizontal" search form template file. 
 * 
 ...
 * Instead you can copy-paste this template ( the "horizontal" folder contains this file and the "css" folder ) 
 ...
 * It will show in the "Search results" dropdown menu as "Custom: horizontal".
 */
?>
<?php do_action( 'gmw_before_search_form_template', $gmw ); ?>

<div class="gmw-form-wrapper <?php echo esc_attr( $gmw['prefix'] ); ?> gmw-fl-form-wrapper gmw-fl-horizontal-custom-form-wrapper">
	...
	<form class="standard-form gmw-form" name="gmw_form" action="<?php echo esc_attr( $gmw_form->get_results_page() ); ?>" method="get" data-id="<?php echo absint( $gmw['ID'] ); ?>" data-prefix="<?php echo esc_attr( $gmw['prefix'] ); ?>">
		...
		<div class="xfield-trigger-wrapper">
			<div class="xfield-trigger" onclick="jQuery(this).closest('form').find('.gmw-search-form-xprofile-fields').slideToggle();">
				<?php echo (!empty($gmw['labels']['search_form']['show_options'])) ? $gmw['labels']['search_form']['show_options'] : __('Show options', 'rehub_framework'); ?>
			</div>
		</div>
