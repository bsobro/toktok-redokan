...
<div <?php post_class( $classes ); ?>>
    ...
    <?php elseif ( $product->is_featured() ) : ?>
        <?php echo apply_filters( 'woocommerce_featured_flash', '<span class="re-ribbon-badge badge_2"><span>' . __( 'Featured!', 'rehub_framework' ) . '</span></span>', $post, $product ); ?>          
    ...
    <div class="grid_desc_and_btn rh-flex-center-align flowhidden">
            ...
            <?php if (rehub_option('woo_btn_disable') != '1'):?> 
                <?php if(!empty($syncitem)):?>
                    ...
                    <?php if ($countoffers > 1) :?>
                        <a class="font90 greencolor" href="<?php the_permalink();?>">+ <?php echo $countoffers - 1; ?> <?php _e('more', 'rehub_framework');?></a>
                    <?php endif;?>

                <?php elseif ( $product->add_to_cart_url() !='') : ?>
                    <?php  echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                        sprintf( '<a href="%s" data-product_id="%s" data-product_sku="%s" class="re_track_btn woo_loop_btn btn_offer_block %s %s product_type_%s"%s %s><i class="fal fa-shopping-bag" aria-hidden="true"></i> %s</a>',
