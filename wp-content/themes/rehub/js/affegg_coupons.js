jQuery(document).ready(function($) {
   ...
   if(affcoupontrigger){
      ...
      $change_mecode.removeClass('masked_coupon woo_loop_btn coupon_btn btn_offer_block wpsm-button').addClass('not_masked_coupon').html( '<i class="fal fa-cut fa-rotate-180"></i><span class="coupon_text">'+ decodeURIComponent(couponcode) +'</span>' );
      $change_mecode.closest('.reveal_enabled').removeClass('reveal_enabled');      
      $.pgwModal({
         titleBar: false,
         mainClassName : 'pgwModal coupon-reveal-popup',
         content: '<div class="coupon_code_in_modal"><div class="re_title_inmodal">' + coupvars.coupontextready + '</div><div class="add_modal_coupon"><div class="text_copied_coupon">' + coupvars.coupontextcopied + '</div></div><div class="coupon_modal_coupon"><input type="text" class="code" value="' + decodeURIComponent(couponcode) + '" readonly=""><span class="buttoncpd"><i class="far fa-check-square"></i></span></div><div class="add_modal_coupon">' + coupvars.coupongoto + '<a href="' + coupondestination + '" target="_blank" rel="nofollow">' + coupvars.couponwebsite + '</a>' + coupvars.couponuseoffer + '<br>' + coupvars.couponorcheck + '</div></div>',
      });
   };
