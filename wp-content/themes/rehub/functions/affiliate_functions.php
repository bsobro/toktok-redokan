	        ...
	        <div class="priced_block clearfix <?php echo $reveal_enabled; echo $coupon_style; ?>">
		    	...
		    	<?php else : ?>
					<?php if(!empty($offer_coupon) && $showme !='price') : ?>
						<?php wp_enqueue_script('zeroclipboard'); ?>
					  	<div class="rehub_offer_coupon not_masked_coupon <?php if(!empty($offer_coupon_date)) {echo $coupon_style ;} ?>" data-clipboard-text="<?php echo $offer_coupon ?>"><i class="fal fa-cut fa-rotate-180"></i><span class="coupon_text"><?php echo $offer_coupon ?></span>
					  	</div>
				  	<?php endif;?>		    		
		        <?php endif; ?>	            	        
	        </div>
			...
			if(class_exists('Woocommerce') && !empty($review_woo_list_links)) :?>
		        <div class="priced_block clearfix">
	                <?php $min_woo_price_count = get_post_meta(get_the_ID(), 'rehub_min_woo_price', true); if ($min_woo_price_count !='' && $showme !='button') : ?>
	                	<p><span class="price_count"><ins><?php echo get_woocommerce_currency_symbol(); echo $min_woo_price_count; ?></ins></span></p>
	                <?php endif ;?>
...
function rehub_create_price_for_list($id) {
	?>
        ...
        <?php elseif ('product' == get_post_type($id)) : ?>
            ...
            <?php if(rehub_option('woo_rhcompare') == 1) {echo'<div class="woo-btn-actions-notext mt10">';echo wpsm_comparison_button(array('class'=>'rhwoosinglecompare', 'id'=>$id)); echo '</div>';} ?>        	
	    <?php endif ;?>	    
...
function set_content_end($content) {
	...
	if ($post->post_type != 'product'){
		if(vp_metabox('rehub_post.rehub_framework_post_type') == 'review') :
			...
			if(vp_metabox('rehub_post.review_post.0.review_post_product_shortcode') == '0') :
				ob_start();
				rehub_get_review();
				$output .= ob_get_clean();
			endif;	
	        ...
	        <div class="single_priced_block <?php echo $reveal_enabled; echo $coupon_style; ?>">
	            <?php if(!empty($offer_price) && $showme !='button') : ?>
            		<span class="single_price_count">
            			<?php echo esc_html($offer_price) ?>
            			<?php if($offer_price_old !='') :?> <del class="font80 rh_opacity_3 fontnormal blockstyle"><?php echo esc_html($offer_price_old) ; ?></del><?php endif ;?>
            		</span>
	            <?php endif ;?>
	    		<?php if($showme !='price') : ?>
