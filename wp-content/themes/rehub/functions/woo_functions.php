...
//CREATE REVIEW META PANEL
$wooreview_metabox  = get_template_directory() . '/admin/metabox/woo_review.php'; 
$wooreview_metabox_obj = new VP_Metabox($wooreview_metabox);

//Remove Image customizer as theme has own resize on fly script
function rehub_remove_woo_image_panel() {     
	global $wp_customize;
	$wp_customize->remove_section( 'woocommerce_product_images' );
} 
add_action( 'customize_register', 'rehub_remove_woo_image_panel', 11 );
...
function woo_ce_pricelist_output(){
	echo do_shortcode('[content-egg-block template=custom/all_offers_logo]' );
}}
...
function woo_cevideo_booking_out(){
	echo '<div class="rh-woo-section-title"><h2 class="rh-heading-icon">'.__('Videos', 'rehub_framework').': <span class="rh-woo-section-sub">'.get_the_title().'</span></h2></div>';	
	echo do_shortcode('[content-egg module=Youtube template=custom/slider]' );
}}
...
if (rehub_option('woo_cart_place') =='1' || rehub_option('woo_cart_place') =='2' || (rehub_option('rehub_header_style') =='header_seven' && rehub_option('header_seven_cart') != '')){
	...
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		<?php if (rehub_option('woo_cart_place') =='1'):?>
			<a class="cart-contents cart_count_<?php echo $woocommerce->cart->cart_contents_count; ?>" href="<?php echo wc_get_cart_url(); ?>"><i class="far fa-shopping-cart"></i> <?php _e( 'Cart', 'rehub_framework' ); ?> (<?php echo $woocommerce->cart->cart_contents_count; ?>) - <?php echo $woocommerce->cart->get_cart_total(); ?></a>		
...
function rh_woo_rating_icons_wrapper($html, $rating, $count){
	if ( 0 < $rating ) {
		$html  = '<div class="rh_woo_star" title="'.sprintf( __( 'Rated %s out of 5', 'rehub_framework' ), esc_html( (float)$rating )).'">';
...
$woo_custom_meta_fields = array(
    ...
    array(
        ...
        'desc'  => __('This option depends on expiration date field, but you can also enable expiration if you have not expiration date', 'rehub_framework'),
        'id'    => 're_post_expired',
        'type'  => 'checkbox'
    ),     
    array(
        'label'=> __('Brand logo url', 'rehub_framework'),
        'desc'  => __('Fallback for brand logo (better to add brand logo in Products - Brands fields)', 'rehub_framework'),
...
function show_rehub_woo_meta_box_side(){
	...
	$badges = apply_filters( 'rehub_product_badges', array(
		'no' => __('No Badge', 'rehub_framework'),		
		'1' => (rehub_option ('badge_label_1') !='') ? rehub_option ('badge_label_1') : __('Editor choice', 'rehub_framework'),
		'2' => (rehub_option ('badge_label_2') !='') ? rehub_option ('badge_label_2') : __('Best seller', 'rehub_framework'), 
		'3' => (rehub_option ('badge_label_3') !='') ? rehub_option ('badge_label_3') : __('Best value', 'rehub_framework'),
		'4' => (rehub_option ('badge_label_4') !='') ? rehub_option ('badge_label_4') : __('Best price', 'rehub_framework'),						
		)
	);
    ...
    echo '<p>'.__('Check this if you want to show badge. You can customize them in theme option', 'rehub_framework').'</p>';	    
}
...
function woo_change_expired_function($expired=''){
	...
	elseif($expired =='no' && $expired_exist == 1){
	...
	function rh_show_vendor_info_single() {
		...
		if($is_vendor){
			...
			if(class_exists( 'WeDevs_Dokan' ) && dokan_get_option( 'contact_seller', 'dokan_general', 'on' ) == 'on'){
				...
				echo ' <a href="'.$store_url.'#dokan-contact-anchor" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub_framework') .'</span></a>';									
				echo '</span>';					
			}
			elseif(is_active_widget( '', '', 'dc-vendor-store-location')){
				echo '<span class="vendor_store_details_contact">';
				echo '<span class="vendor_store_owner_label">@ <span class="vendor_store_owner_name">'.get_the_author_meta('display_name') .'</span></span>';
				$class = ( !is_user_logged_in() && rehub_option( 'userlogin_enable' ) == '1' ) ? ' act-rehub-login-popup' : '';						
				echo ' <a href="'.$store_url.'#wcmp-vendor-contact-widget-top" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub_framework') .'</span></a>';									
				echo '</span>';	
			}	
			elseif(class_exists( 'BuddyPress' ) ) {
				...
				if ( bp_is_active( 'messages' )){
					...
					$class = (!is_user_logged_in() && rehub_option('userlogin_enable') == '1') ? ' act-rehub-login-popup' : '';
						echo ' <a href="'.$link.'" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub_framework') .'</span></a>';			
				}
				echo '</span>';		
			}
...
if( class_exists( 'WeDevs_Dokan' ) ) {
	...
	*/
	...
	function custom_dokan_gwm_save_location( $store_id, $dokan_settings ){
		if( !$store_id OR !function_exists('gmw_update_user_location') ) return;
		...
		if( !empty( $address_arr ) OR !empty( $find_address ) ) {
			...
			gmw_update_user_location( $store_id, $address, true );
		}
	}
	...
	function custom_dokan_remove_section( $sections ){
		if(!empty($sections) && is_array($sections)){
	        foreach ( $sections as $key=>$section ) {
	            if ( isset( $section['id'] ) && 'dokan_appearance' == $section['id'] ) {
	                unset( $sections[$key] );
	            }
	        }
		}
...
if( class_exists('WCMp')) {
	...
	function wcmp_remove_rh_vendor_dashboard_template_settings($settings_tab_options){
		if(isset($settings_tab_options['sections']['wcmp_vendor_shop_template'])){
			unset($settings_tab_options['sections']['wcmp_vendor_shop_template']);
		}
	...
	function wcmprh_loop_sold_by(){
		...
		if ('Enable' === get_wcmp_vendor_settings('sold_by_catalog', 'general') && apply_filters('wcmp_sold_by_text_after_products_shop_page', true, $post->ID)){
			...
			$sold_by_text = apply_filters('wcmp_sold_by_text', __('Sold By', 'rehub_framework'));
			?><small class="wcvendors_sold_by_in_loop"><span><?php echo $sold_by_text ?></span> <?php echo $sold_by; ?></small><br /><?php
		}
	}
		...
		function rh_wcmp_gwm_save_adress($user_id = '') {
			if(function_exists('gmw_update_user_location')){
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST)) {
					$formatted_address = (!empty($_POST['_store_location'])) ? $_POST['_store_location'] : '';
					if( $formatted_address ) {
						gmw_update_user_location( $vendor_id, $formatted_address, true );
					}
				}
			}
		}
