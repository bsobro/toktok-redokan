...
function RH_get_wishlist( $post_id, $wishlistadd = '',$wishlistadded = '', $wishlistremoved = '' ) { 
    ...
    $output .= '<span class="flowhidden cell_wishlist">';   
        if ( AlreadyWish( $post_id ) ) { // already liked, set up unlike addon
            ...
            $output .= '<span class="heartplus'.$onlyuser_class.'" data-post_id="'.$post_id.'" data-informer="'.$temp.'">'.$outputtext.'</span>';
        }   
    $output .= '</span>';
...
if(!function_exists('rh_wishlist_shortcode')){
    ...
    add_shortcode('RH_wishlist', 'rh_wishlist_shortcode');
}

if (!function_exists('RhGetUserFavorites')){
function RhGetUserFavorites($icon = 'no') {
    ...
    if (!empty($wishlistids)){
        ...
        if($icon == 'yes'){
            echo '<span class="fal fa-heart position-relative">';
                echo '<span class="rh-icon-notice">'.count($wishlistids).'</span>';
            echo '</span>';
        }else{
            $args = array( 
                'post_status' => 'publish', 
        ...
        _e('There is nothing in your wishlist', 'rehub_framework');
    }    
}
...
function getShopLikeButton( $user_id ) {
    ...
    if ( AlreadylikedShop( $user_id ) ) { // already liked, set up unlike addon
        $output .= '<span class="favorshop_like"><i class="fas fa-heart"></i></span>';
        $output .= ' <span class="count">'.$likes.'</span></span>';
    } else { // normal like button
        $output .= '<span class="favorshop_like"><i class="far fa-heart"></i></span>';
        $output .= ' <span class="count">'.$likes.'</span></span>';
    }
