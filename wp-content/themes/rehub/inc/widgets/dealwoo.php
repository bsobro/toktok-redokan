...
function widget( $args, $instance ) {
	...
	if ( $loop->have_posts() ) :
		...
		if ( $title ) : ?><div class="title"><?php echo $title; ?></div><?php endif; ?>
		<div class="woo_spec_widget text-center">
			...
			<?php $post_id = $loop->post->ID; ?>
				<?php if ( 'product' == $dealtype && class_exists('woocommerce')):?>
				    ...
				    <figure class="centered_image_woo">
						<?php
							if ( $_product->is_featured() ) :
								echo apply_filters( 'woocommerce_featured_flash', '<span class="onfeatured">' . __( 'Featured!', 'rehub_framework' ) . '</span>', $loop->post, $_product );
							endif; 
							if ( $_product->is_on_sale() ) : 
								...
								if ( $percentage && $percentage > 0 && !$_product->is_type( 'variable' ) ) {
									...
									$sales_html = apply_filters( 'woocommerce_sale_flash', '<span class="onsale'. $featured .'">' . esc_html__( 'Sale!', 'rehub_framework' ) . '</span>', $loop->post, $_product );
								}
								echo $sales_html;
							endif; 
						?>
				    ...
				    <div class="woo_loop_desc"> 
				    	<h3>      
					        <a class="<?php echo getHotIconclass($post_id); ?>" href="<?php echo $product_link ;?>"<?php echo $target_blank; ?>>
		        	...
		        	<?php else:?>
			        	<?php 
			        		...
			        		if ( $sale_price_dates_to ) {
								...
								$month = $expireddate[1];
								$day  = $expireddate[2];
								if($offer_coupon_date){$day = $day +1;}
			        		} else {
			        			$year = '';
			        		}
						?>	        	
	...
	function form( $instance ) {
		...
		<p>
			...
			<small><?php _e('By default, widget shows latest product which is on sale, you can specify product ID to overwrite this', 'rehub_framework'); ?></small>
		</p>
		...
		<p>
			...
			<br /><small><?php _e('By default, widget shows countdown base on Sale price dates of product. You can enable fake timer (always shows 12 hours)', 'rehub_framework'); ?></small>
		</p>
		...
		<p>
			...
			<br /><small><?php _e('By default, widget shows real progress bar based on stock status, you can enable fake bar', 'rehub_framework'); ?></small>
		</p>
	<?php
	}
}
