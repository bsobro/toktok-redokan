...
define('RH_MAIN_THEME_VERSION', '7.4.3');
...
function rehub_framework_register_scripts() {
	...
	wp_register_style('fontawesome', get_template_directory_uri() . '/vafpress-framework/public/css/vendor/font-awesome.min.css', array(), '5.0.9');
	...
	wp_register_script('custom_scroll', get_template_directory_uri() . '/js/custom_scroll.js', array('jquery'), '1.0.1', true);
	...
	wp_register_script( 'rehubcompare', get_template_directory_uri() . '/js/comparechart.js', array('jquery', 'rehub'), '1.0.4', true );
...
require ( locate_template( '/inc/customizer.php' ) );
	...
	if(function_exists('bp_get_profile_field_data')){
		if(bp_is_user()){
			...
			if (function_exists('gmw_get_location')){
				$gmw_member_info = gmw_get_location('user', $bpuserid);
				if (!empty( $gmw_member_info ) && is_object($gmw_member_info)){
					...
					$jsonload["address"] = array(
						...
						"addressRegion" => $gmw_member_info->region_name,
						"postalCode" => $gmw_member_info->postcode,
						"addressCountry" => $gmw_member_info->country_name,	
...
function rehub_admin_error_notice() {
    ...
    if ( ! get_user_meta($user_id, 'ignore_notices_rehub741') ) {
		if ($page=='rehub' || $page=='rehub-support' || $page=='rehub-plugins' || $page=='rehub-demos' || $page=='vpt_option' ) {
			$class = "error";
			$message = 'This is important update with some changes. Check article first <a href="https://wpsoul.com/rehub-7-4/" target="_blank">Rehub 7.4.1</a>' ;
...
function rehub_nag_ignore() {
    ...
    if ( isset($_GET['rehub_nag_ignore']) && '0' == $_GET['rehub_nag_ignore'] ) {
        add_user_meta($user_id, 'ignore_notices_rehub741', 'true', true);
