			        ...
			        <div class="flowhidden pb10 pt15 border-grey-bottom">               
			            <div class="floatleft mobileblockdisplay mb15 offer_thumb">   
			                <a rel="nofollow" target="_blank" href="<?php echo esc_url($product->add_to_cart_url()) ?>" class="re_track_btn">
			                	<?php 
			                		$term_ids =  wp_get_post_terms($product->get_id(), 'store', array("fields" => "ids")); 
			                		$brand_url = '';
			            ...
			            <div class="floatright buttons_col pl20 rtlpr20 wpsm-one-half-mobile wpsm-column-last">
							...
							<a href="<?php the_permalink();?>" class="font80 lineheight15 clearbox floatright"><?php _e('Details', 'rehub_framework');?></a>	                                 
			            </div>                                  
...
function wpsm_toprating_shortcode( $atts, $content = null ) {
	...
	if(isset($atts['id']) || isset($atts['postid'])):
    	...
    	?>
            ...
            <div class="review_visible_circle top_rating_block<?php if(isset($atts['full_width']) && $atts['full_width']=='1') : ?> full_width_rating<?php else :?> with_sidebar_rating<?php endif;?> list_style_rating">
                ...
                <div class="top_rating_item" id='rank_<?php echo $i?>'>                    
                    <div class="product_image_col">                        	
                        <figure><?php echo re_badge_create('ribbon'); ?>
                        	<span class="rank_count"><?php if (($i) == '1') :?><i class="far fa-trophy-alt"></i><?php else:?><?php echo $i?><?php endif ?></span>
	            ...
	            <tr class="top_rating_item" id='rank_<?php echo $i?>'>
	                <?php if ($first_column_enable):?>
	                    <td class="product_image_col"><?php echo re_badge_create('tablelabel'); ?>
	                        <figure>
	                            <?php if (!is_paged() && $first_column_rank) :?><span class="rank_count"><?php if (($i) == '1') :?><i class="far fa-trophy-alt"></i><?php else:?><?php echo $i?><?php endif ?></span><?php endif ?>                        
...
if($btntext !='') :
	...
	$output .='<i class="far fa-external-link-square"></i></div>';
return $output;
}
...
if (is_user_logged_in()) {
	...
	if ( function_exists('bp_notifications_get_notifications_for_user')) {
		...
		$notification_bp_item .='<li class="bp-profile-edit-menu-item menu-item"><a href="'.bp_core_get_user_domain( $user_id ).'"><i class="fas fa-cogs"></i></i><span>'. __("Edit Profile", "rehub_framework") .'</span></a></li>';		
    ...
    $output .= '<ul class="user-dropdown-intop-menu">';
        ...
        $output .= '</li>';
        if ($profile_url) :
        	$output .= '<li class="user-profile-link-intop menu-item"><a href="'. esc_url(get_the_permalink($profile_url)) .'"><i class="fas fa-user-circle"></i><span>'. __("My profile", "rehub_framework") .'</span></a></li>';
        endif;
        if ($sumbit_url) :
        	$output .= '<li class="user-addsome-link-intop menu-item"><a href="'. esc_url(get_the_permalink($sumbit_url)) .'"><i class="fas fa-cloud-upload"></i><span>'. __("Submit a Post", "rehub_framework") .'</span></a></li>';
        endif; 
        if ($edit_url) :
        	$output .= '<li class="user-editposts-link-intop menu-item"><a href="'. esc_url(get_the_permalink($edit_url)) .'"><i class="fas fa-edit"></i><span>'. __("Edit My Posts", "rehub_framework") .'</span></a></li>';
        endif;  
        if (defined('wcv_plugin_dir')) :
        	...
        	if (!empty($redirect_to)){
	        	$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. esc_url($redirect_to) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub_framework") .'</span></a></li>';        	
        	}
        endif; 
        if( class_exists( 'WeDevs_Dokan' ) ) :
        	$is_vendor = dokan_is_user_seller( $user_id );
        	if($is_vendor) :
	        $output .= '<li class="user-editshop-link-intop menu-item"><a href="'. dokan_get_navigation_url() .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub_framework") .'</span></a></li>'; 
	        endif; 
        endif; 
        if( class_exists('WCMp')) :
        	...
        	if($is_vendor) :
        		...
        		if ($dashlink > 0):
	        		$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. get_permalink($dashlink) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub_framework") .'</span></a></li>'; 
	        	endif;         		
    		endif;
        ...
        $output .= '<li class="user-logout-link-intop menu-item"><a href="'. wp_logout_url( home_url()) .'"><i class="far fa-lock-alt"></i><span>'. __("Log out", "rehub_framework") .'</span></a></li>';
$output .= '</ul></div>';
} else {
	if(get_option('users_can_register')) :
		if (empty ($loginurl)):
			if ($wrap =='a'):
				$output .= '<a class="act-rehub-login-popup'.$as_button.$class_show.'" data-type="login" href="#"><i class="'.$icon_class.'"></i><span>'.__("Login / Register", "rehub_framework").'</span></a>';
			else:
				$output .= '<span class="act-rehub-login-popup'.$as_button.$class_show.'" data-type="login"><i class="'.$icon_class.'"></i><span>'.__("Login / Register", "rehub_framework").'</span></span>';
			endif;
		else:
			$output .= '<span class="act-rehub-login-popup'.$as_button.$class_show.'" data-type="url" data-customurl="'.esc_url($loginurl).'"><i class="'.$icon_class.'"></i><span>'.__("Login / Register", "rehub_framework").'</span></span>';
		endif;
	else:
		$output .= '<a class="act-rehub-login-popup'.$as_button.$class_show.'" data-type="restrict" href="#"><i class="'.$icon_class.'"></i><span>'.__("Login / Register is disabled", "rehub_framework").'</span></a>';
	endif;	
...
function wpsm_comparison_button( $atts, $content = null ) {
	...
	$out = '<span';   
    $out .=' class="wpsm-button wpsm-button-new-compare addcompare-id-'.$postid.' '.$atts['color'].' '.$atts['size'].''.$compare_active.$class_show.'" data-addcompare-id="'.$postid.'"><i class="fal re-icon-compare"></i><span class="comparelabel">'.__("Add to compare", "rehub_framework").'</span></span>';
    return $out;
}
...
function wpsm_tax_archive_shortcode( $atts, $content = null ) {
	...
	$args = array( 'hide_empty' => false, 'order' => 'ASC', 'taxonomy'=> $taxonomy, 'number'=> $number, 'child_of' => $child_of);
	...
	if($type == 'alpha') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach( $letter_keyed_terms as $letter => $terms ) {
				$term_letter_links .= '<li><a href="#'.$letter.'" class="font120 blackcolor rehub_scroll">'.$letter.'</a></li>';

				$term_titles .= '<div class="single-letter mt20 mb20 pb10 border-grey-bottom"><a href="#" name="'.$letter.'"></a><div class="letter_tag fontbold font120 lineheight20">'.$letter.'<div class="return_to_letters cursorpointer floatright font80"><span class="rehub_scroll" data-scrollto="#top_ankor"><i class="far fa-angle-up"></i></span></div></div></div>';
				...
				foreach( $terms as $term ) {
					...
					elseif ( $taxonomy == 'product_cat' && $show_images == 1 ) {
							$brandimageid = get_woocommerce_term_meta( $term->term_id, 'thumbnail_id', true ); 
						  	if ($brandimageid) {
					...
					$term_titles .= '<div id="taxonomy-'. $term->term_id .'" class="tax-item text-center floatleft mt10 mr10"><a class="single-letter-link" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub_framework' ), $term->name ) ) . '">' . $thumbnail . '<h5 class="mt5 mb0 font80 fontnormal">'. $term->name . '</h5></a></div>';
				}
	...
	elseif ($type == 'compact') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				$term_titles .= '<a class="'.$classitem.' compact-tax-link tax-item text-center floatleft mt10 mr10" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub_framework' ), $term->name ) ) . '"><h5 class="mt0 mb0 font80 lineheight15">'. $term->name . '</h5></a>';
			}
	...
	elseif ($type == 'compactbig') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach( $terms as $term ) {
				$term_titles .= '<a class="'.$classitem.' col_item mb10 text-center rh-main-bg-hover blackcolor rh-cartbox big-tax-link" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub_framework' ), $term->name ) ) . '"><div class="rehub-main-font">'. $term->name . '</div></a>';
			}
	...
	elseif ($type == 'logo') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if ($thumbnail){
					$term_titles .= '<a class="'.$classitem.' col_item mb10 rh_centered_image text-center rh-cartbox pt10 pb10 pl10 pr10 logo-tax-link" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub_framework' ), $term->name ) ) . '">'. $thumbnail . '</a>';					
				}
			}
			return '<div class="'.$classcol.' rh-flex-eq-height">'.$term_titles.'</div>';	
		}
...
function rh_wcv_vendorslist_flat( $atts ) {
		...
		<div class="tabledisplay mb20">
			<form id="search-sellers" role="search" method="get" class="celldisplay search-form floatleft mb10">
				...
				<button type="submit" alt="'. __('Search', 'rehub_framework') .'" value="'. __('Search', 'rehub_framework') .'" class="btnsearch"><i class="fal fa-search"></i></button>
			</form>
...
function rh_compare_icon($atts, $content = null ) {
	if (rehub_option('compare_page') != '' || rehub_option('compare_multicats_toggle') == 1) {	
		$output = '<span class="re-compare-icon-toggle">';
			$output .= '<i class="fal fa-balance-scale" aria-hidden="true"></i>';
            ...
            <?php if($video == 1 && !empty($post_image_videos)):?>   
	            <?php foreach($post_image_videos as $key=>$video):?>
	                <a href="<?php echo esc_url($video);?>" data-rel="rehub_postthumb_gallery_<?php echo $random_key;?>" target="_blank" class="mb10 rh_videothumb_link" data-poster="<?php echo parse_video_url(esc_url($video), 'maxthumb'); ?>" data-thumb="<?php echo parse_video_url(esc_url($video), 'hqthumb'); ?>">
						<img src="<?php echo parse_video_url(esc_url($video), 'hqthumb'); ?>" height="<?php echo $height;?>" alt="" />
	                </a>                               
	            <?php endforeach;?> 
            <?php endif;?>                       
        </div>
...
function rh_get_post_videos($atts, $content = NULL){
    ...
    <?php if(!empty($post_image_videos) ) :?>
    	<?php $random_key = rand(0, 50);?>
		<?php $post_image_videos = array_map('trim', explode(PHP_EOL, $post_image_videos));?> 
        <div class="<?php echo $class;?> modulo-lightbox rh_post_videos mt15 mb20" data-galleryid="rhvid_<?php echo $random_key;?>">   
            <?php foreach($post_image_videos as $key=>$video):?>
                <a href="<?php echo esc_url($video);?>" data-rel="rehub_postvid_gallery_<?php echo $random_key;?>" target="_blank" class="mb10 inlinestyle rh_videothumb_link" data-poster="<?php echo parse_video_url(esc_url($video), 'maxthumb'); ?>" data-thumb="<?php echo parse_video_url(esc_url($video), 'hqthumb'); ?>"> 
					<img src="<?php echo parse_video_url(esc_url($video), 'maxthumb'); ?>" alt="" />
                </a>                               
            <?php endforeach;?>                       
        </div>
        <?php  wp_enqueue_script('modulobox'); wp_enqueue_style('modulobox');?>
...
add_shortcode('rh_get_post_videos', 'rh_get_post_videos');
}
...
function wpsm_reviewbox( $atts, $content = null ) {
    ...
    if($compact && $id){
    	...
    	if($score){
    		...
    		elseif($compact == 'text'){
			    $out = '<span class="lineheight20 rh-flex-center-align rehub-main-font"><span class="score_text_r font110 mr5"><span class="fontbold">'.$score.'</span>/10</span>
			    		<span class="greycolor font70">('.__("Expert Score", "rehub_framework").')</span>
			    		</span>';
    		}  
    		elseif($compact == 'square'){
			    $out = '<div class="review-top">
                    <div class="overall-score">
                        ...
                        <span class="overall-text">'.__("Expert Score", "rehub_framework").'</span>
                    </div> 
                </div> ';
    		}    		
    ...
    elseif($woobox && $id){
		...
		$out = '<div class="rate_wide_block mobileblockdisplay rh-flex-center-align '.$cols.'" id="rate_wide_block_'.$rand_id.'">';
			$out .='<style scoped>#rate_wide_block_'.$rand_id.'{background-image: url('.$image_url.');     background-size: cover;}</style>';
			...
			if(!empty($pros) || !empty($cons) ) {
				$out .= '<div class="rh_col prosconswrap">';
					if(!empty($pros)):
						$out .='<div class="prosblock lineheight15"><div class="title_pros mb15 font110 fontbold upper-text-trans">'.__('+ Positives:', 'rehub_framework').'</div><ul>';		
					...
					if(!empty($cons) && !empty($thecriteria[0]['review_post_name'])):
						$out .='<div class="consblock lineheight15"><div class="title_cons mb15 font110 fontbold upper-text-trans">'.__('- Negatives:', 'rehub_framework').'</div><ul class="mb0">';		
			...
			if(empty($thecriteria[0]['review_post_name']) && !empty($cons) ) {
				$out .= '<div class="rh_col conswrap">';
					$out .='<div class="consblock lineheight15"><div class="title_cons mb15 upper-text-trans">'.__('- Negatives:', 'rehub_framework').'</div><ul>';		
					$consvalues = explode(PHP_EOL, $cons);
					foreach ($consvalues as $consvalue) {
						if(!empty($consvalue)){
							$out .='<li>'.esc_html($consvalue).'</li>';						
		...
		$out = '<div class="rate_bar_wrap"><div class="review-top"><div class="overall-score">';
			...
			$out .= '<span class="overall">'.$total_score.'</span><span class="overall-text">'.__('Expert Score', 'rehub_framework').'</span></div>';
			$out .='<div class="review-text"><span class="review-header">'.esc_html($title).'</span><p>'.esc_html($description).'</p></div></div><div class="rh-line mb10 mt10"></div>';
			if (!empty($criterias))  {
