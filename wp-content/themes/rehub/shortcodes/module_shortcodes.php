			...
			<div class="col-feat-grid item-<?php echo $col_number;?>" style="background-image: url('<?php echo $image_url ;?>');">
				<a href="<?php the_permalink();?>" class="feat_overlay_link"></a> 
		  		<div class="feat-grid-overlay text_in_thumb">
		    		...
		    		<?php if ($col_number == 1) :?>
		    		<div class="post-meta">
                		<span class="date_ago"><i class="far fa-clock"></i> <?php printf( __( '%s ago', 'rehub_framework' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) ); ?></span>		    		
...
?>
	...
	$i=1; if ( $wp_query->have_posts() ) : ?>
		...
		<?php $gridtypeclass = ($gridtype == 'compact') ? ' eq_grid post_eq_grid' : ' grid_woo';?>  
		<?php $gridtypetemplate = ($gridtype == 'compact') ? 'woogridcompact' : 'woogridpart';?>  
		<div class="rh-flex-eq-height products <?php echo $infinitescrollwrap; echo $col_wrap; echo $gridtypeclass; ?>" data-filterargs='<?php echo $jsonargs;?>' data-template="<?php echo $gridtypetemplate;?>" id="<?php echo $containerid;?>" data-innerargs='<?php echo $json_innerargs;?>'>                   
...
$ajaxoffset = (int)$show + (int)$offset;
$additional_vars = array();
$additional_vars['inner_link_enable'] = $inner_link_enable; 
ob_start(); 
?>
<?php rehub_vc_filterpanel_render($filterpanel, $containerid, $taxdrop, $taxdroplabel, $taxdropids);?>
<?php
...
?>
	<?php 
		...
		$json_innerargs = json_encode($additional_vars);
	?>
	<div class="wpsm_recent_posts_list mb0 <?php echo $center_class; echo $col_wrap; echo $infinitescrollwrap;?>" data-filterargs='<?php echo $jsonargs;?>' data-template="simplepostlist" id="<?php echo $containerid;?>" data-innerargs='<?php echo $json_innerargs;?>'>
<?php if ( $wp_query->have_posts() ) : ?>
...
if( !function_exists('wpsm_searchbox_function') ) {
	...
	<div class="custom_search_box<?php if ($label):?> flat_style_form<?php endif;?>">
		...
		<?php } else { ?>
			<form  role="search" class="flowhidden" method="get" id="rh-custom-search" action="<?php echo home_url( '/' ); ?>">
			  ...
			  <i class="far fa-arrow-right inside-search"></i>
			  <button type="submit" class="wpsm-button <?php if ($label):?>rehub_main_btn<?php else:?><?php echo $color?><?php endif;?>"><?php if ($label):?><?php echo esc_attr($label);?><?php else:?><i class="fal fa-search"></i><?php endif;?></button>
			</form>
