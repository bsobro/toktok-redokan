...
class Hybrid_Provider_Model_OpenID extends Hybrid_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once Hybrid_Auth_dokan::$config["path_libraries"] . "OpenID/LightOpenID.php";

		// An error was occurring when proxy wasn't set. Not sure where proxy was meant to be set/initialized.
		Hybrid_Auth_dokan::$config['proxy'] = isset(Hybrid_Auth_dokan::$config['proxy'])?Hybrid_Auth_dokan::$config['proxy']:'';

		$hostPort = parse_url( Hybrid_Auth_dokan::$config["base_url"], PHP_URL_PORT);
		$hostUrl = parse_url( Hybrid_Auth_dokan::$config["base_url"], PHP_URL_HOST);
		...
		$this->api = new LightOpenID( $hostUrl, Hybrid_Auth_dokan::$config["proxy"] );
	}
	...
	*/
	function loginBegin()
	{
		...
		Hybrid_Auth_dokan::redirect( $this->api->authUrl() );
	}
	...
	*/
	function loginFinish()
	{
		...
		Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->providerId}.user", $this->user );
	}
	...
	*/
	function getUserProfile()
	{
		...
		$this->user = Hybrid_Auth_dokan::storage()->get( "hauth_session.{$this->providerId}.user" ) ;
