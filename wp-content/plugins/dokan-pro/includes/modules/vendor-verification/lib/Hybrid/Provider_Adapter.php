...
/**
 * Hybrid_Provider_Adapter is the basic class which Hybrid_Auth_dokan will use
 ...
 * 
 * Hybrid_Auth_dokan will automatically load Hybrid_Provider_Adapter and create
...
class Hybrid_Provider_Adapter
{
	...
	*/
	function factory( $id, $params = array() )
	{
		...
		if( isset( $this->config["wrapper"] ) && is_array( $this->config["wrapper"] ) ){
			...
			require_once Hybrid_Auth_dokan::$config["path_providers"] . $this->id . ".php" ;
	...
	*/
	function login()
	{
		...
		foreach( Hybrid_Auth_dokan::$config["providers"] as $idpid => $params ){
			Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$idpid}.hauth_return_to"    );
			Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$idpid}.hauth_endpoint"     );
			Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$idpid}.id_provider_params" );
		}
		...
		if (empty(Hybrid_Auth_dokan::$config["base_url"])) {
			...
			$Hybrid_Auth_dokan_URL_BASE = $url;
		} else {
			$Hybrid_Auth_dokan_URL_BASE = Hybrid_Auth_dokan::$config["base_url"];
		}
		...
		$this->params["login_start"] = $Hybrid_Auth_dokan_URL_BASE . ( strpos( $Hybrid_Auth_dokan_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.start={$this->id}&hauth.time={$this->params["hauth_time"]}";

		# for default HybridAuth endpoint url hauth_login_done_url
		# 	auth.done   required  the IDp ID
		$this->params["login_done"]  = $Hybrid_Auth_dokan_URL_BASE . ( strpos( $Hybrid_Auth_dokan_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";

        if( isset( $this->params["hauth_return_to"] ) ){
            Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->id}.hauth_return_to", $this->params["hauth_return_to"] );
        }
        if( isset( $this->params["login_done"] ) ){
            Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->id}.hauth_endpoint" , $this->params["login_done"] ); 
        }
        Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->id}.id_provider_params" , $this->params );

		// store config to be used by the end point 
		Hybrid_Auth_dokan::storage()->config( "CONFIG", Hybrid_Auth_dokan::$config );

		// move on
		Hybrid_Logger::debug( "Hybrid_Provider_Adapter::login( {$this->id} ), redirect the user to login_start URL." );

		Hybrid_Auth_dokan::redirect( $this->params["login_start"] );
	}
	...
	*/
	...
	{ 
		...
		$callback_url = Hybrid_Auth_dokan::storage()->get( "hauth_session.{$this->id}.hauth_return_to" );

		// remove some unneeded stored data
		Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$this->id}.hauth_return_to"    );
		Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$this->id}.hauth_endpoint"     );
		Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$this->id}.id_provider_params" );

		// back to home
		Hybrid_Auth_dokan::redirect( $callback_url );
	}
	...
	*/
	...
	{ 
		if( isset( Hybrid_Auth_dokan::$config["providers"][$id] ) ){
			return Hybrid_Auth_dokan::$config["providers"][$id];
		}
