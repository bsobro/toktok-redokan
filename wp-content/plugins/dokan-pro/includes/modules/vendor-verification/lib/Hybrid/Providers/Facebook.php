	...
	*/
	function initialize() 
	{
		...
		if ( ! class_exists('FacebookApiException', false) ) {
			require_once Hybrid_Auth_dokan::$config["path_libraries"] . "Facebook/base_facebook.php";
			require_once Hybrid_Auth_dokan::$config["path_libraries"] . "Facebook/facebook.php";
		}
		
		if ( isset ( Hybrid_Auth_dokan::$config["proxy"] ) ) {
			BaseFacebook::$CURL_OPTS[CURLOPT_PROXY] = Hybrid_Auth_dokan::$config["proxy"];
		}
	...
	*/
	function loginBegin()
	{
		...
		foreach ($optionals as $parameter){
			if( isset( $this->config[$parameter] ) && ! empty( $this->config[$parameter] ) ){
				...
				if($parameter == "auth_type"){
					...
					Hybrid_Auth_dokan::storage()->set('fb_auth_nonce', $nonce);
				}
			}
		}
        ...
        if( isset( $this->config[ 'force' ] ) && $this->config[ 'force' ] === true ){
            ...
            Hybrid_Auth_dokan::storage()->set( 'fb_auth_nonce', $parameters[ 'auth_nonce' ] );
        }
		...
		Hybrid_Auth_dokan::redirect( $url );
	}
	...
	*/
	...
	{ 
		...
		if(isset($_REQUEST['auth_nonce'])){
			...
			$nonce = Hybrid_Auth_dokan::storage()->get('fb_auth_nonce');
			
			//Delete the nonce
			Hybrid_Auth_dokan::storage()->delete('fb_auth_nonce');
