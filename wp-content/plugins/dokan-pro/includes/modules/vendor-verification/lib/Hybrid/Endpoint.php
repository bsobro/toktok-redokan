...
class Hybrid_Endpoint {
	...
	*/
	public static function processOpenidXRDS()
	{
		...
		(
			...
			str_replace(
				array("<", ">", "\"", "'", "&"), array("&lt;", "&gt;", "&quot;", "&apos;", "&amp;"), 
				Hybrid_Auth_dokan::getCurrentUrl( false )
			),
	...
	*/
	public static function processOpenidRealm()
	{
		...
		(
			"{X_XRDS_LOCATION}",
			htmlentities( Hybrid_Auth_dokan::getCurrentUrl( false ), ENT_QUOTES, 'UTF-8' ) . "?get=openid_xrds&v=" . Hybrid_Auth_dokan::$version,
	...
	*/
	public static function processAuthStart()
	{
		...
		if( ! Hybrid_Auth_dokan::storage()->get( "hauth_session.$provider_id.hauth_endpoint" ) ) {
		...
		$hauth = Hybrid_Auth_dokan::setup( $provider_id );
	...
	public static function authInit()
	{
		if ( ! Hybrid_Endpoint::$initDone) {
			...
			try {
				...
				// Check if Hybrid_Auth_dokan session already exist
				...
				Hybrid_Auth_dokan::initialize( $storage->config( "CONFIG" ) ); 
			}
			catch ( Exception $e ){
				Hybrid_Logger::error( "Endpoint: Error while trying to init Hybrid_Auth_dokan: " . $e->getMessage()); 
