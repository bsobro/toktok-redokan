...
class Hybrid_Provider_Model_OAuth1 extends Hybrid_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once Hybrid_Auth_dokan::$config["path_libraries"] . "OAuth/OAuth.php";
		require_once Hybrid_Auth_dokan::$config["path_libraries"] . "OAuth/OAuth1Client.php";
		...
		if( isset( Hybrid_Auth_dokan::$config["proxy"] ) ){
			$this->api->curl_proxy = Hybrid_Auth_dokan::$config["proxy"];
		}
	}
	...
	*/
	function loginBegin()
	{
		...
		Hybrid_Auth_dokan::redirect( $this->api->authorizeUrl( $tokens ) );
	}
