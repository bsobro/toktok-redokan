...
abstract class Hybrid_Provider_Model
{
	...
	function __construct( $providerId, $config, $params = NULL )
	{
		...
		if( ! $params ){
			$this->params = Hybrid_Auth_dokan::storage()->get( "hauth_session.$providerId.id_provider_params" );
		}
		else{
			$this->params = $params;
		}
		...
		$this->endpoint = Hybrid_Auth_dokan::storage()->get( "hauth_session.$providerId.hauth_endpoint" );
	...
	*/
	...
	// --------------------------------------------------------------------
   	...
   	/**
	* generic logout, just erase current provider adapter stored data to let Hybrid_Auth_dokan all forget about it
	*/
	function logout()
	{
	...
	*/ 
	...
	{
		return (bool) Hybrid_Auth_dokan::storage()->get( "hauth_session.{$this->providerId}.is_logged_in" );
	}
	...
	*/ 
	public function setUserConnected()
	{
		...
		Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->providerId}.is_logged_in", 1 );
	}
	...
	*/ 
	public function setUserUnconnected()
	{
		...
		Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->providerId}.is_logged_in", 0 ); 
	}
	...
	*/ 
	...
	{
		if( $value === NULL ){
			return Hybrid_Auth_dokan::storage()->get( "hauth_session.{$this->providerId}.token.$token" );
		}
		else{
			Hybrid_Auth_dokan::storage()->set( "hauth_session.{$this->providerId}.token.$token", $value );
		}
	}
	...
	*/ 
	...
	{
		Hybrid_Auth_dokan::storage()->delete( "hauth_session.{$this->providerId}.token.$token" );
	}
	...
	*/ 
	...
	{ 
		Hybrid_Auth_dokan::storage()->deleteMatch( "hauth_session.{$this->providerId}." );
	}
}
